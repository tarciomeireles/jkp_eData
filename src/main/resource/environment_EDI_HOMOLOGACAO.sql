WITH NewParameter AS (
  ...
  INSERT INTO environment.parameter_value(
	parameter_code_id, "value", description)
		SELECT parameter_code_id, 100407, 'JKP RESOURCE_ID TX: Transmissão' FROM NewParameter;
...
WITH NewParameter AS (
  ...
  INSERT INTO environment.parameter_value(
	parameter_code_id, "value", description)
		SELECT parameter_code_id, 100409, 'JKP RESOURCE_ID NOT: Notificação' FROM NewParameter;
...
WITH NewParameter AS (
  ...
  INSERT INTO environment.parameter_value(
	parameter_code_id, "value", description)
		SELECT parameter_code_id, 100410, 'JKP RESOURCE_ID RX: Recepção' FROM NewParameter;
